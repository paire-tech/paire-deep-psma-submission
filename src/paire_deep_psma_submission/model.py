...
def load_model_from_file(weights_path: Union[str, Path], device: str = "cpu") -> DynUNet:
    """
    Load a model from a weights file.

    Args:
        weights_path: The path to the model weights.
...
def load_models(weights_dir: Union[str, Path], prefix: str, device: str = "cpu") -> List[DynUNet]:
    ...
    Args:
        weights_dir: The directory containing the model weights.
        prefix: The prefix of the model weights.
    ...
    for weights_path in weights_paths:
        log.info("Loading model weights from '%s'", weights_path)
        model = load_model_from_file(weights_path, device)
