...
class Equald(T.MapTransform):
    ...
    def __call__(self, data: Dict) -> Dict:
        data = dict(data)
        for key, dst_key, value in self.key_iterator(data, self.dst_keys, self.value):
            data[dst_key] = (data[key] == value) * 1
        return data


# Mostly copied from monai.transforms.AsDiscreted, with a new parameter `dst_keys` to specify output keys
class AsDiscreted(T.MapTransform):
    def __init__(
        self,
        keys: KeysCollection,
        ...
        **kwargs: Any,
        ...
        super().__init__(keys, allow_missing_keys)
        self.argmax = ensure_tuple_rep(argmax, dim=len(self.keys))
        self.to_onehot = []
        for flag in ensure_tuple_rep(to_onehot, dim=len(self.keys)):
        ...
        self.threshold = []
        for flag in ensure_tuple_rep(threshold, dim=len(self.keys)):
            if isinstance(flag, bool):
                raise ValueError("`threshold_values=True/False` is deprecated, please use `threshold=value` instead.")
            self.threshold.append(flag)

        self.rounding = ensure_tuple_rep(rounding, dim=len(self.keys))
        self.dst_keys = ensure_tuple_rep(dst_keys or self.keys, dim=len(self.keys))
        self.converter = T.AsDiscrete()
        self.converter.kwargs = kwargs

    def __call__(self, data: Dict) -> Dict[str, Any]:
        d = dict(data)
        for key, dst_key, argmax, to_onehot, threshold, rounding in self.key_iterator(
            d, self.dst_keys, self.argmax, self.to_onehot, self.threshold, self.rounding
        ):
            d[dst_key] = self.converter(d[key], argmax, to_onehot, threshold, rounding)
        return d
